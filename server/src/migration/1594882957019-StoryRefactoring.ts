...
export class StoryRefactoring1594882957019 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    await queryRunner.query(
      `CREATE TABLE "temporary_user" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "uid" varchar, "username" varchar NOT NULL, "password" varchar NOT NULL, "email" varchar, "isSuperuser" boolean NOT NULL DEFAULT (0), "createdAt" datetime NOT NULL DEFAULT (datetime('now')), "updatedAt" datetime NOT NULL DEFAULT (datetime('now')), "provider" varchar NOT NULL DEFAULT ('porfirevich'), "photoUrl" varchar, "isBanned" boolean NOT NULL DEFAULT (0))`,
      undefined,
    );
    await queryRunner.query(
      `INSERT INTO "temporary_user"("id", "uid", "username", "password", "email", "isSuperuser", "createdAt", "updatedAt", "provider", "photoUrl") SELECT "id", "uid", "username", "password", "email", "isSuperuser", "createdAt", "updatedAt", "provider", "photoUrl" FROM "user"`,
      undefined,
    );
